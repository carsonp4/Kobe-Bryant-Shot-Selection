...
rf_tg <- grid_regular(mtry(c(1,ncol(train))), min_n(), levels=2)
...
xg_tg <- dials::grid_max_entropy(xg_params, size = 4)
...
bart_tg <- grid_regular(trees(), levels = 2)
...
nb_tg <- grid_regular(Laplace(),smoothness(), levels = 2)
...
knn_tg <- grid_regular(neighbors(),levels = 2)
...
svm_tg <- grid_regular(rbf_sigma(), cost(),levels = 2)
...
nn_tg <- grid_regular(hidden_units(range=c(1,100)), levels=2)
